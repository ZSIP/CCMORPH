...
def method_2(profile, begin_no, end_no, min_profile_points=20):
    retVal = {"M2_D_high": None, "M2_D_low": None, "status": "OK"}
    ...
    if (
        ...
        D_min = min(D)
        D_max = max(D)
        retVal["M2_D_high"] = begin_no + D.index(D_max)
        retVal["M2_D_low"] = begin_no + D.index(D_min)
    else:
        set_status(
            retVal,
...
def method_1(profile, begin_no, end_no, min_profile_points=20, elevation_threshold=3, profile_id=0, debug=False):
    retVal = {"M1_D_high": None, "M1_D_low": None, "status": "OK", "debug": None}
