...
points_first_file = join(points_input_path, csv_points["first"]) # main file if more fils
profiles_input_path = join(config["paths"]["base"], config["paths"]["input"]["profiles"])
...
points = pd.read_csv(
    points_first_file, encoding="utf-8", sep=csv_points["sep"], skipinitialspace=True # todo
)
...
for name in profile_files:
    with spiner(text=f"{counter} / {profile_files_count} -> {basename(name)}", spinner="dots"):
        ...
        result = {
            ...
            "first_zero_x": first_zero.x_geo.values[0] if len(first_zero) > 0 else np.nan,
            "first_zero_y": first_zero.y_geo.values[0] if len(first_zero) > 0 else np.nan,
            "first_zero_elevation": first_zero.elevation.values[0] if len(first_zero) > 0 else np.nan,
            ...
            "last_zero_elevation": last_zero.elevation.values[0] if len(last_zero) > 0 else np.nan,
            
            "bottom_x": bottom.x_geo.values[0] if len(bottom) > 0 else np.nan,
            "bottom_y": bottom.y_geo.values[0] if len(bottom) > 0 else np.nan,
            "bottom_elevation": bottom.elevation.values[0] if len(bottom) > 0 else np.nan,
            ...
            "to_bottom_volume": get_volume(points_distance, csv, first_zero_id, bottom_id, True),
...
results.to_csv(join(config["paths"]["base"], config["paths"]["output"]["finall"], csv_output["first"]), sep=csv_output["sep"])
...
bottom_points.rename(columns = {"bottom_elevation":"elevation", "bottom_id": "point_id"}, inplace = True)
...
top_points.rename(columns = {"top_elevation":"elevation", "top_id": "point_id"}, inplace = True)
first_zero_points = gpd.GeoDataFrame(results[["profile_id", "first_zero_id", "first_zero_elevation"]], geometry=gpd.points_from_xy(results.first_zero_x, results.first_zero_y))
first_zero_points.rename(columns = {"first_zero_elevation":"elevation", "first_zero_id": "point_id"}, inplace = True)
last_zero_points = gpd.GeoDataFrame(results[["profile_id", "last_zero_id", "last_zero_elevation"]], geometry=gpd.points_from_xy(results.last_zero_x, results.last_zero_y))
last_zero_points.rename(columns = {"last_zero_elevation":"elevation", "last_zero_id": "point_id"}, inplace = True)

bottom_points.to_file(join(shapes_output_path, "bottomPoints"), crs=config["shape"]["crs"])
top_points.to_file(join(shapes_output_path, "topPoints"), crs=config["shape"]["crs"])
first_zero_points.to_file(join(shapes_output_path, "firstZeroPoints"), crs=config["shape"]["crs"])
last_zero_points.to_file(join(shapes_output_path, "lastZeroPoints"), crs=config["shape"]["crs"])
